...
| `MONGODB_URI` | — | MongoDB Atlas `mongodb+srv://` connection string. When set, all application state uses MongoDB instead of SQLite. |
| `MONGODB_DB` | `tgdrive` | MongoDB database name. |
...
When using MongoDB Atlas on Render, Atlas must accept connections from Render's
outbound IP ranges. Add those ranges in **Atlas → Security → Network Access**, or
use `0.0.0.0/0` while testing. Keep a strong database password and never disable
