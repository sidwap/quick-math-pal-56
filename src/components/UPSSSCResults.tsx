...
interface UPSSSCResult {
  ...
  // UPSSSC-specific
  upssscGrossWords: number;
  upssscPenalty: number;
  upssscNetCorrectWords: number;
  upssscFinalSpeed: number;
  admissibleErrors: number;
  excessErrors: number;
  isQualified: boolean;
}
...
const UPSSSCResults = ({ result, comparison, originalText, testDuration, onStartNewTest }: Props) => {
    ...
    <div className="min-h-screen bg-background p-2 sm:p-4">
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="bg-primary text-primary-foreground rounded-xl p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <p className="font-bold">UPSSSC Junior Assistant {result.language} Typing</p>
            </div>
            <div>
        ...
        <div className="bg-secondary/50 border border-border rounded-xl overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-2 md:grid-cols-5 gap-3">
            <div className="text-center p-3 bg-background rounded-lg border">
              <p className="text-2xl font-bold text-primary">{result.typedKeystrokes}</p>
              <p className="text-xs text-muted-foreground">Total Keystrokes</p>
            </div>
            <div className="text-center p-3 bg-background rounded-lg border">
              <p className="text-2xl font-bold text-foreground">{result.upssscGrossWords.toFixed(1)}</p>
              <p className="text-xs text-muted-foreground">Gross Words (÷5)</p>
            </div>
            <div className="text-center p-3 bg-background rounded-lg border">
              <p className="text-2xl font-bold text-destructive">{result.upssscPenalty}</p>
              <p className="text-xs text-muted-foreground">Penalty Words</p>
            </div>
            <div className="text-center p-3 bg-background rounded-lg border">
              <p className="text-2xl font-bold text-green-600">{result.upssscNetCorrectWords.toFixed(1)}</p>
              <p className="text-xs text-muted-foreground">Net Correct Words</p>
            </div>
            <div className="text-center p-3 bg-background rounded-lg border">
              <p className={`text-2xl font-bold ${result.isQualified ? 'text-green-600' : 'text-destructive'}`}>
                {result.upssscFinalSpeed.toFixed(2)}
              </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          ...
          <StatCard label="Admissible Errors" value={result.admissibleErrors} icon={AlertTriangle} valueColor="text-amber-500" />
          <StatCard label="Excess Errors" value={result.excessErrors} icon={XOctagon} valueColor="text-red-500" />
          <StatCard label="Accuracy %" value={`${comparison.stats.accuracy}`} icon={Percent} valueColor="text-primary" />
          <StatCard label={`Min Speed: ${minSpeed} WPM`} value={Number(result.upssscFinalSpeed).toFixed(2)} icon={Gauge} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 gap-3 sm:gap-4">
          <StatCard 
            label="Gross Speed (Word-based)" 
            value={Number(result.grossSpeed).toFixed(2)} 
            icon={Gauge} 
            valueColor="text-blue-600"
          />
          <StatCard 
            label="Net Speed (Word-based)" 
            value={Number(result.netSpeed).toFixed(2)} 
            icon={Gauge} 
        ...
        <div className="bg-secondary/50 border border-border rounded-xl overflow-hidden">
          ...
          <div className="text-center text-sm sm:text-base p-4 space-y-3">
            <div className="bg-background rounded-lg p-3 border">
              ...
              <p className="text-primary font-bold">{result.typedKeystrokes} ÷ 5 = {result.upssscGrossWords.toFixed(1)}</p>
            </div>
            <div className="bg-background rounded-lg p-3 border">
              <p className="font-semibold">Step 2: Errors beyond 5 admissible → Penalty = (Errors - 5) × 5</p>
              <p className="text-primary font-bold">
                Total Errors: {comparison.stats.totalErrors} | Admissible: 5 | 
                Excess: {result.excessErrors} | Penalty: {result.excessErrors} × 5 = {result.upssscPenalty}
              </p>
            </div>
            <div className="bg-background rounded-lg p-3 border">
              <p className="font-semibold">Step 3: Net Correct Words = Gross Words - Penalty</p>
              <p className="text-primary font-bold">
                {result.upssscGrossWords.toFixed(1)} - {result.upssscPenalty} = {result.upssscNetCorrectWords.toFixed(1)}
              </p>
            </div>
            <div className="bg-background rounded-lg p-3 border">
              <p className="font-semibold">Step 4: Final Speed = Net Correct Words ÷ Time (minutes)</p>
              <p className="text-primary font-bold">
                {result.upssscNetCorrectWords.toFixed(1)} ÷ {testDuration / 60} = {result.upssscFinalSpeed.toFixed(2)} WPM
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-secondary/50 border border-border rounded-xl overflow-hidden">
          ...
          <div className="text-center text-sm sm:text-base p-4">
            <p className="font-semibold">
              शुद्धता = (शुद्ध शब्दों की संख्या × 100) ÷ कुल शब्दों की संख्या
            </p>
            <p className="mt-2">
              Accuracy = (Correct Words × 100) ÷ Total Words
            </p>
        ...
        <div className="bg-secondary/50 border border-border rounded-xl overflow-hidden">
          ...
          <div className="text-sm sm:text-base space-y-2 p-4">
            <p>UPSSSC Junior Assistant qualifying criteria:</p>
            <ul className="list-disc list-inside ml-4 space-y-1">
              <li><strong>English: Minimum 30 WPM</strong> (5 keystrokes = 1 word)</li>
              <li><strong>Hindi: Minimum 25 WPM</strong> (5 keystrokes = 1 word)</li>
              <li><strong>Duration: 5 Minutes</strong></li>
              <li><strong>5 Admissible Errors</strong> - Each extra error = 5-word penalty</li>
            </ul>
            <div className={`mt-3 p-3 rounded-lg ${result.isQualified ? 'bg-green-500/10 border border-green-500' : 'bg-destructive/10 border border-destructive'}`}>
              <p className="font-bold text-center">
                Your Speed: {result.upssscFinalSpeed.toFixed(2)} WPM | Required: {minSpeed} WPM
              </p>
        ...
        <div className="bg-secondary/50 border border-border rounded-xl overflow-hidden">
          ...
          <div className="text-xs sm:text-sm space-y-2 p-4">
            <p className="font-semibold text-destructive">Full Mistakes:</p>
            <ol className="list-decimal list-inside space-y-1 ml-4">
              <li>
                <span className="text-violet-500 font-semibold">Omission (छोड़ना)</span> - For every omission of a word/figure.
              </li>
              <li>
                <span className="text-red-500 font-semibold">Substitution (प्रतिस्थापन)</span> - For every wrong word/figure.
              </li>
              <li>
                <span className="text-orange-500 font-semibold">Addition (जोड़ना)</span> - For every extra word not in passage.
              </li>
              <li>
                <span className="text-red-500 font-semibold">Spelling Error (वर्तनी त्रुटि)</span> - For spelling mistakes.
              </li>
              <li>
                <span className="text-red-500 font-semibold">Repetition (दोहराव)</span> - For word repetition.
              </li>
            </ol>
            <div className="mt-3 p-3 bg-amber-500/10 border border-amber-500 rounded-lg">
              <p className="font-semibold text-amber-700 dark:text-amber-400">
                ⚠️ Note: Up to 5 errors are admissible. For every extra error beyond 5, a penalty of 5 words is deducted.
              </p>
            </div>
          </div>
        </div>
